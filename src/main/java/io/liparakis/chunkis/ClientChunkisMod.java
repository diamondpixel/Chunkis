...
public class ClientChunkisMod implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ChunkisMod.LOGGER.info("Chunkis Client initializing...");
        ...
        ChunkisMod.LOGGER.info("Chunkis Client initialized successfully.");
    }
    ...
    private static void processChunkDelta(ChunkDeltaPayload payload, ClientWorld world) {
        ...
        try {
            ...
            if (!(chunk instanceof ChunkisDeltaDuck deltaDuck)) {
                ...
                if (errorCount.get() == 0) {
                    ChunkisMod.LOGGER.warn("Chunk at ({}, {}) is not ChunkisDeltaDuck: {}",
            ...
            if (ChunkisMod.LOGGER.isDebugEnabled()) {
                ChunkisMod.LOGGER.debug(
    ...
    private static void scheduleBlockRerendering(
        ...
        if (count < RENDER_BATCH_THRESHOLD) {
            ...
            // For large deltas, still iterate but use array access pattern for better cache locality
    ...
    private static void logErrorThrottled(String message, Throwable cause) {
        ...
        if ((errors & (ERROR_LOG_INTERVAL - 1)) == 1) { // Works if ERROR_LOG_INTERVAL is power of 2
            if (cause != null) {
                ChunkisMod.LOGGER.error("{} - Error #{} (every {}th logged)",
    ...
    public static long[] getMetrics() {
        return new long[]{
                ...
                totalDecodeTimeNanos.get(),
                totalBlocksChanged.get()
        };
    }
    ...
    public static void resetMetrics() {
        ...
        totalDecodeTimeNanos.set(0);
        totalBlocksChanged.set(0);
        errorCount.set(0);
    }
}