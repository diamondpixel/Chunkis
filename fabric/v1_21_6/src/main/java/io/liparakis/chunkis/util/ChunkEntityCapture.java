 ...
 *
 * <p>Captures all non-player entities within a chunk's boundaries and serializes
 * them to NBT format for delta storage. This preserves entities across chunk
 * unloads and world reloads.
 *
 * <p><b>Entity Filtering Rules:</b>
 * <ol>
 ...
 *   <li>Entities physically outside chunk bounds excluded (prevents duplication)</li>
 * </ol>
 *
...
public final class ChunkEntityCapture {
     ...
     *
     * <p>Queries all entities within the chunk's bounding box, filters using
     * {@link #shouldCaptureEntity}, serializes each to NBT, and stores in the
     * delta for persistence.
     *
     * <p><b>Entity Types Captured:</b>
     * <ul>
     ...
     * </ul>
     *
     * <p><b>Thread Safety:</b> Must be called on server thread as it queries
    ...
    public static void captureAll(WorldChunk chunk, ChunkDelta<?, NbtCompound> delta, ServerWorld world) {
        ...
        var capturedEntities = filterAndSerializeEntities(candidateEntities, chunkPosition);
     ...
     *
     * <p>Creates a bounding box spanning the entire chunk from world bottom to
     * top, then queries all entities within that box. The query is broad and
     * returns candidates that require further filtering.
     *
     * <p><b>Thread Safety:</b> Must execute on server thread as world entity
     ...
     *
     * <p>Applies filtering rules to exclude players, removed entities, passengers,
    ...
    private static List<NbtCompound> filterAndSerializeEntities(
        ...
        for (var entity : candidateEntities) {
            if (shouldCaptureEntity(entity, chunkPosition)) {
                var nbt = serializeEntity(entity);
     ...
     *
     * <p>Uses Minecraft's NBT write view system for proper serialization with
     ...
     *
     * <p>Applies strict filtering rules to prevent data duplication and ensure
     ...
     *   <li><b>Passengers:</b> Excluded (saved with vehicle to prevent duplication)</li>
     *   <li><b>Coordinate check:</b> Must be physically within chunk bounds</li>
     * </ol>
     *
     * <p>The coordinate check uses floor division and bit shifting for optimal
     ...
     *
     * <p>Uses direct coordinate math with floor division (bit shift) to convert
